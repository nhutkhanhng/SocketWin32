...
public:
	...
	friend std::ostream& operator << (std::ostream &os, OutputMemoryBitStream stream)
	{
		for (int i = 0; i < stream.mBitHead; i++)
			os << stream.mBuffer[i];
...
public:
	...
	friend std::ostream& operator << (std::ostream &os, InputMemoryBitStream inputMemory)
	{
		std::cout << inputMemory.mBitHead << "\t";

		for (int i = 0; i < inputMemory.mBitHead; i++)
		{
			os << inputMemory.mBuffer[i];
		}
